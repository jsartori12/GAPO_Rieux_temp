...
class GeneticAlgoBase:
    ...
    def mutate(self, ind, lista_fixed, mutation_type):
        lista_len_seq = list(range(0, len(ind)))
        lista_fixed = [x - 1 for x in lista_fixed]
        ...
        if mutation_type == "esm":
            ...
            command = f"python mutation_esm.py --seq {''.join(ind)} --position {position} --temperature {self.esm_tmp}"
            subprocess.run(command, stdout=subprocess.PIPE, shell=True)
            
            with open("completed_sequence.txt", "r") as file:
                content = file.read()
