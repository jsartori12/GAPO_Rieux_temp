...
def Generate_random_population(starting_pose, pop_size, fixed_residues_list, chains):
    ...
    positions_starting_pose = list(range(1,vector_size))
...
def apt_esm(seq, temperature=1.0):

    command = f"python test_apt_function.py --seq {seq} --temperature {temperature}"
    subprocess.run(command, stdout=subprocess.PIPE, shell=True)
    
    with open("avg_probability.txt", "r") as file:
        content = file.read()
